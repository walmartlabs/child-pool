...
exports.prototype.send = function(message, callback) {
  ...
  function fork() {
    ...
    worker.process.on('exit', function(code, signal) {
      if ((process.platform === 'linux' && code === null) || (process.platform === 'win32' && signal === null)) {
        
        // when child process crashes
        activeTasks--;
        self.workers = _.without(self.workers, worker);

        try {
          if (worker.callback) {
            worker.callback(new Error('Process crashed'), null);
          } else {
            self.emit('error', new Error('Process crashed'));
          }
        } finally {
          process.nextTick(function() {
            if (queue.length) {
